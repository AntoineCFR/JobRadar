...
MATCH_INSTRUCTIONS = (
    ...
    "5. Languages — GRADUATED by the GAP between the REQUIRED level (offer) and the "
    "candidate's OWN declared level for THAT language. Read the candidate's level from "
    "CANDIDATE.languages — NEVER assume or hardcode a level, NEVER infer a need from "
    "location. Compare on the CEFR scale (A1<A2<B1<B2<C1<C2; native=C2). A required "
    "language is SATISFIED when the candidate's declared level meets or exceeds it -> no "
    "penalty (whatever the language). When the requirement is ABOVE the candidate's "
    "declared level:\n"
    "   - 2+ CEFR steps above (e.g. declared B1 vs required C1/C2, or no knowledge of a "
    "required fluent/business language) -> NEAR-DISQUALIFYING: high-severity blocker "
    "capping the score at ~5-12. If the ROLE is ALSO a poor fit, the two stack and the "
    "score MUST be <= 10 (typically 5-8).\n"
    "   - exactly 1 CEFR step above (e.g. declared B1 vs required B2) -> a REAL but "
    "SURMOUNTABLE gap: subtract meaningfully but do NOT hard-cap; the score can still "
    "reach 25-45 if the rest fits ('envisageable').\n"
    "   A deterministic post-check re-enforces these caps from the DECLARED levels, so "
    "keep your reasoning consistent with the candidate's real per-language levels.\n"
    "   REALISM: never advise reaching C1/C2 in a few months of courses/immersion — a "
    "2-3 level jump is unrealistic short-term; say so honestly rather than proposing an "
    "illusory language plan.\n"
...
CALIBRATE_INSTRUCTIONS = (
    ...
    "fluent/business language the candidate has no knowledge of) = near-disqualifying "
    "(cap ~5-12; if the role ALSO mismatches, stack to <= 10), high-severity blocker; "
    "required exactly 1 step above = significant but SURMOUNTABLE (NO hard cap; can stay "
    "25-45 if the rest fits); required at or below the declared level = satisfied, no "
    "blocker.\n"
